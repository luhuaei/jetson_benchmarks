...
def main():
    ...
    if args.all:
        ...
        if not download_err:
            ...
            if not error_log:
                model.remove()
    elif args.model_name == 'zh-clip-text':
        model = run_benchmark_models(csv_file_path=csv_file_path, model_path=model_path, precision=precision, benchmark_data=benchmark_data)
        download_err = model.execute(read_index=10)
        if not download_err:
            _, error_log = model.report()
            if not error_log:
                model.remove()
    elif args.model_name == 'zh-clip-vision':
        model = run_benchmark_models(csv_file_path=csv_file_path, model_path=model_path, precision=precision, benchmark_data=benchmark_data)
        download_err = model.execute(read_index=11)
        if not download_err:
            _, error_log = model.report()
            if not error_log:
                model.remove()
        ...
        model = run_benchmark_models(csv_file_path=csv_file_path, model_path=model_path, precision=precision, benchmark_data=benchmark_data)
        download_err = model.execute(read_index=12)
        if not download_err:
            _, error_log = model.report()
            if not error_log:
                model.remove()
